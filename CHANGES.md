...
- TODO: opdrachtgeverfilter: alleen een door de gebruiker zelf gekozen rij krijgt de grijze achtergrond; een parent die alleen aanstaat doordat zijn kinderen aanstaan toont een streepje (indeterminate), geen grijs. Geen dikgedrukte labels en geen gefade checkbox meer (WCAG-contrast / geen regel-wrap). De top-3 van elke filtergroep (opdrachtgever, rol, labels, merk) is nu vast op count: een optie aanvinken verspringt of verdringt niets meer — een keuze buiten de top-3 komt er onderaan bij. De filterzijbalk springt niet meer naar boven na een filterwijziging (scrollpositie van de sidebar-box wordt bewaard)
...
- TODO: fix dat de aantallen naast de opdrachtgevers nu wel updaten met de andere actieve filters
