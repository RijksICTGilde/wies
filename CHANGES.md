...
- NLDD: "Wie zit waar?" (de homepagina) toont inzetten nu als kaarten in plaats van een lijst, met een weergave-switch (Persoon / Opdracht) en een sorteermenu. De persoon-kaart toont avatar + naam + (bij één opdracht) opdrachtnaam en opdrachtgever, anders het aantal opdrachten; de opdracht-kaart toont naam, opdrachtgever, het volledige team als avatars en de einddatum. De weergave-switch is een `nldd-segmented-control`, het sorteermenu een expandable `nldd-menu-bar-item` met `nldd-menu` (`data-href`-navigatie zoals het gebruikersmenu). Kaarten zijn `nldd-card` met `href` + `hx-get` naar het side panel (hx_link_guard blokkeert de navigatie). Paginatie loopt op plaatsingen maar kaarten zijn per persoon/opdracht; `placement_toolbar.js` dedupliceert een groep die de paginagrens overlapt
