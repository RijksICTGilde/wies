...
- 427: fix an opdracht without roles missing from `/opdrachten/`; filtering on rol or expertise still hides it
- 427: fix the counts in the opdrachtgever "Meer" sheet ignoring the other active filters, so they disagreed with the numbers in the sidebar
- 427: fix editing an opdracht whose Business Manager is not in the BDM group: the field came up empty and saving failed on a field you never touched
- 580: fix the missing `<h1>` on Wie zit waar?, Aanvragen and Mijn profiel: the toolbar replaced the page heading, so the heading structure started at an `<h3>` in the sidebar
- 576: fix opdracht descriptions losing their line breaks and their "Toon meer" toggle, and move the description back into the data list instead of a bare paragraph above the tabs
- 553: fix Escape marking the onboarding wizard as done for good; only "Overslaan" in the title bar does that now
- 544: the user list shows roles as a tag again and filters on rol, expertise, thema and merk from a sheet, with rol now a multi-select
- 603: fix an opdracht without roles not showing up on `/opdrachten/`: the list only selected assignments with an unfilled open role, so a freshly created one was nowhere to be found. Filtering on rol or expertise still hides it
- 603: an opdracht card without roles now says "Nog geen rollen" and always carries a period row, so it lines up with the others instead of looking oddly short
...
- 572: het focusherstel na een htmx-swap grijpt niet meer in na een muisinteractie. Wie klikt weet waar hij is; de focus verplaatsen naar een element waar de muis niet is levert hem niets op. Wie met het toetsenbord werkt raakt zijn plek wél kwijt als de focus valt, en daar blijft het herstel dus staan. Het script kijkt naar de laatste invoer (`keydown` tegenover `pointerdown`), dezelfde vraag die de browser zelf beantwoordt voor `:focus-visible`. Gevolg: dat de focus na het opslaan van een inline bewerking op de bewerkknop landt, geldt vanaf nu alleen bij toetsenbordbediening. Het focussen gebruikt daarnaast `preventScroll`, zodat het herstel nooit een sprong in de pagina kan veroorzaken
- 572: het focusherstel na een htmx-swap zet geen `tabindex` meer op de vervangen container, maar focust het eerste bedienbare element in de nieuwe inhoud. De container is een component (`nldd-page`) en dus een shadow host, en bij zo'n element bepaalt de `tabindex` van de host of zijn inhoud meedoet in de tabvolgorde. Gemeten op het zijpaneel: met dat attribuut liep Tab nog één ronde en bleef daarna op `<body>` hangen, en kwam Shift+Tab de inhoud helemaal niet meer in. Het attribuut werd ook nooit opgeruimd, dus één swap brak het paneel voor de rest van de paginalevensduur
...
- TBD: na het opslaan van een inline bewerking springt de focus terug naar het potloodje waar je begon, in plaats van naar het begin van de pagina. Het formulier wordt weggeswapt, dus de knop die focus had verdwijnt en de volgende Tab begon weer bovenaan: lastig bij toetsenbordnavigatie. De leesweergave krijgt na een opslag `autofocus` op de bewerkknop, wat htmx zelf oppakt. Ook weg: twee dode CSS-regels in `modals.css` die de focusring helemaal verwijderden (WCAG 2.4.7)
