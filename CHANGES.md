...
- TODO: (BM-exploratie) nieuw hoofdmenu-item "Business management" met een eigen zijmenu en twee subpagina's: "Bezetting" (tijdlijn van 3 maanden terug t/m 1 jaar vooruit die per collega toont wie werk nodig heeft — op volgorde van urgentie, met een totaal aantal onbezette uren per week bovenaan; klik op een rij opent het collega-zijpaneel) en "Prognose" (capaciteitsgrafiek: per week over dezelfde horizon de beschikbare capaciteit — som contracturen — versus de ingeplande uren als vlakdiagram; het witte gat ertussen is onbezette capaciteit, die naar de toekomst toe oploopt naarmate plaatsingen aflopen; hover toont capaciteit/ingepland/onbezet per week. Grafiek met lokaal gevendorde uPlot-library). Geen rechten-check (demo)
- TODO: (BM-exploratie) "Uren per week" hoort nu bij de rol/dienst in plaats van bij de plaatsing, zodat ook een openstaande aanvraag (dienst zonder consultant) uren draagt. De Prognose-grafiek toont daardoor een derde reeks "Aanvragen", bovenop het ingeplande gestapeld; in een deel van de horizon kan de gevraagde capaciteit (ingepland + aanvragen) boven de beschikbare capaciteit uitkomen (hover toont dan "Tekort" in plaats van "Onbezet")
- TODO: (BM-exploratie) de Prognose is nu een relatieve staafgrafiek per maand met een jaar-keuzelijst: per maand toont een 100%-gestapelde balk welk deel van de beschikbare capaciteit is ingepland (groen) en aangevraagd (oranje), met de vrije capaciteit als rest. De uPlot-vlakdiagram is vervangen
