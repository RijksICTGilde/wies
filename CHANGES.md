...
- 585: het decoratieve vinkje in een filterrij gebruikt nu het `decorative`-attribuut van `nldd-checkbox` in plaats van een zelfgebouwde `aria-hidden="true" tabindex="-1"`. Die twee bereikten de shadow root niet, dus het `<input>` daarbinnen bleef bestaan en programmatisch focusbaar: focusbare inhoud in een tak die voor hulpsoftware als verborgen is aangemerkt, binnen de rij-knop die zelf al `role="checkbox"` draagt. Met `decorative` rendert de component dat veld helemaal niet meer en blijft alleen het vakje over. Gemeten met axe-core: dit ruimt 30 schendingen op (`aria-hidden-focus` en `nested-interactive`, allebei serious) op de plaatsingen- en opdrachtenlijst
