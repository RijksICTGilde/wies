...
describe("nodeMatches", function () {
  it("matches by label substring (case-insensitive)", function () {
    assert.ok(TreeState.nodeMatches({ label: "Belastingdienst" }, "belasting"));
    assert.ok(TreeState.nodeMatches({ label: "Belastingdienst" }, "BELASTING"));
  });

  it("matches by abbreviation", function () {
    assert.ok(
      TreeState.nodeMatches(
        { label: "Ministerie van Financien", abbreviations: ["MinFin"] },
        "minfin",
      ),
    );
  });
  ...
  it("returns false when no match", function () {
    assert.ok(
      !TreeState.nodeMatches(
        { label: "Belastingdienst", abbreviations: ["BD"] },
        "xyz",
      ),
    );
  });
...
describe("collectMatches", function () {
  it("returns matching nodes", function () {
    var data = realisticTree();
    var matches = TreeState.collectMatches(data, "belasting");
    assert.equal(matches.length, 1);
    assert.equal(matches[0].label, "Belastingdienst");
  });

  it("skips group nodes even when label matches", function () {
    var data = realisticTree();
    var matches = TreeState.collectMatches(data, "ministerie");
    // "Ministeries" is a group → skipped. "Ministerie van Financien" matches.
    // "Ministerie van BZK" matches. Self-node skipped.
    var labels = matches.map(function (m) {
      return m.label;
    });
    assert.ok(!labels.includes("Ministeries"));
    assert.ok(labels.includes("Ministerie van Financien"));
    assert.ok(labels.includes("Ministerie van BZK"));
  });

  it("skips self-nodes", function () {
    var data = realisticTree();
    var matches = TreeState.collectMatches(data, "financien");
    var ids = matches.map(function (m) {
      return String(m.id);
    });
    assert.ok(!ids.includes("self-10"));
    assert.ok(ids.includes("10"));
  });

  it("searches recursively into children", function () {
    var data = realisticTree();
    var matches = TreeState.collectMatches(data, "afdeling");
    assert.equal(matches.length, 1);
    assert.equal(matches[0].label, "Afdeling IT");
  });

  it("returns empty for no matches", function () {
    var data = realisticTree();
    var matches = TreeState.collectMatches(data, "xyznonexistent");
