...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not db_path.exists():
            self.stdout.write(self.style.WARNING(f"Database file does not exist: {db_path}"))
            return
        ...
        try:
            db_path.unlink()
            self.stdout.write(self.style.SUCCESS(f"Successfully removed database file: {db_path}"))
        except OSError as e:
