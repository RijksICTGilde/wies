...
class PlacementListView(ListView):
    ...
    # weergave-switch; `value` becomes ?groep=.
    GROUPBY_DEFAULT = "person"
    GROUPBY_OPTIONS = [
        {"value": "person", "label": "Persoon", "icon": "person"},
        {"value": "assignment", "label": "Opdracht", "icon": "clipboard"},
    ]
    ...
    def get_template_names(self):
        ...
        if "HX-Request" in self.request.headers:
            ...
            return ["parts/filter_and_card_container_placements.html"]
    ...
    def get_context_data(self, **kwargs):
        ...
        # assignment panel. Set on the related objects so the card can link straight.
        assignment_ids = set()
        ...
        # Weergave-switch options, each with its URL + active flag.
        context["groupby_options"] = [
            {**opt, "url": _groupby_url(opt["value"]), "active": opt["value"] == active_group}
            for opt in self.GROUPBY_OPTIONS
        ]

        # The switch/count shows the active view's label + a distinct count over the
        # full filtered set (not the current page).
