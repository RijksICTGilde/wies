...
from wies.core.models import (
    User, Colleague, LabelCategory, Label,
    Assignment, Service, Placement, Ministry, Skill
)
...
class LabelFilteringAndDisplayTest(TestCase):
    ...
    def setUp(self):
        ...
        self.merken_category, _ = LabelCategory.objects.get_or_create(
            name='Merk',
            defaults={'color': '#0066CC'}
        )
        self.skills_category, _ = LabelCategory.objects.get_or_create(
            name='Skills',
            defaults={'color': '#00AA00'}
        )

        self.rig_label, _ = Label.objects.get_or_create(
            name='Rijks ICT Gilde',
            category=self.merken_category
        )
        self.rc_label, _ = Label.objects.get_or_create(
            name='Rijksconsultants',
            category=self.merken_category
        )
        self.iir_label, _ = Label.objects.get_or_create(
            name='I-Interim Rijk',
            category=self.merken_category
        )

        self.python_label, _ = Label.objects.get_or_create(
            name='Python',
            category=self.skills_category
        )
        self.django_label, _ = Label.objects.get_or_create(
            name='Django',
            category=self.skills_category
        )
    ...
    def test_empty_filter_state_handling(self):
        ...
        no_label_colleague = Colleague.objects.create(
            name='No Label Colleague',
            email='nolabel@colleague.com',
            source='wies'
        )

        # Create placement for colleague without labels
        placement = Placement.objects.create(
            colleague=no_label_colleague,
            service=self.service,
            source='wies'
        )
    ...
    def test_multiple_colleagues_same_label_all_shown(self):
        ...
        self.assertIn('Colleague One', content)
        self.assertIn('Colleague Three', content)
