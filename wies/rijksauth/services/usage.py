...
def get_usage_stats(now: datetime | None = None) -> dict:
    ...
    windowed = success.filter(timestamp__gte=daily_cutoff_utc)
    total_logins_90d = windowed.count()

    rows = (
        windowed.annotate(day=TruncDate("timestamp", tzinfo=DASHBOARD_TZ))
        .values("day")
        .annotate(c=Count("user_email", distinct=True))
    )
    counts_by_day: dict[date, int] = {row["day"]: row["c"] for row in rows}

    daily_active_users: list[tuple[date, int]] = [
    ...
    max_daily = max((c for _, c in daily_active_users), default=0)
